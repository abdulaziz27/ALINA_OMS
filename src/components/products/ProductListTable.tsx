    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-[24px] border border-pink-100/60 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <button
            ...
            className="bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded-xl cursor-pointer transition text-xs flex items-center gap-1.5 shadow-sm"
          >
          ...
          <button
            ...
            className="bg-pink-50/50 hover:bg-pink-50 text-pink-500 border border-pink-100 font-semibold py-2 px-3 rounded-xl cursor-pointer transition text-xs flex items-center gap-1.5"
          >
      ...
      <div className="bg-white p-4 rounded-[24px] border border-pink-100/60 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs font-sans">
        <div className="flex flex-col gap-1.5 text-left">
          <span className="font-extrabold text-gray-400 uppercase tracking-widest text-xs">Klasifikasi Produk</span>
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg font-bold transition-all text-[11px] cursor-pointer ${
                    active 
                      ? 'bg-pink-500 text-white shadow-sm font-black scale-102' 
        ...
        <div className="flex flex-col gap-1.5 text-left w-full md:w-64">
          <span className="font-extrabold text-gray-400 uppercase tracking-widest text-xs">Varian Spesifik ({selectedClassification === 'All' ? 'Semua' : selectedClassification})</span>
          <select
      ...
      <div className="bg-white rounded-[32px] border border-pink-100/70 p-5 shadow-sm space-y-4">
        <div className="flex justify-between items-center pb-2 border-b border-pink-50">
          <div className="text-left">
            ...
            <span className="text-sm text-gray-400 block font-medium">Ketuk produk untuk mengubah detail atau mencetak label Code128.</span>
          </div>
          <span className="text-sm bg-pink-50 text-pink-500 px-3 py-0.5 rounded-full font-mono font-bold">
                ...
                return (
                  ...
                  >
                    <td className="py-3.5 px-3">
                      ...
                      <div className="text-sm font-mono text-pink-500 font-bold tracking-wider">{p.SKU}</div>
                    </td>
                    ...
                    <td className="py-3.5 px-3 text-center">
                      ...
                      {urgent && <div className="text-xs text-red-500 font-bold mt-0.5 animate-pulse">LOW STOCK</div>}
                    </td>
                    <td className="py-3.5 px-3 text-center">
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="space-y-1">
                    <span className="text-xs font-black font-mono text-pink-500 bg-pink-50 py-0.5 px-2.5 rounded-md uppercase tracking-wider">
                      {p.SKU}
                    </span>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                    <span className={`text-xs font-black uppercase px-2.5 py-1 rounded-full text-center ${
                      p.Status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                    }`}>
                      {p.Status === 'Active' ? 'Aktif' : 'Non-Aktif'}
                    </span>
                ...
                <div className="flex justify-between items-end border-t border-dashed border-pink-100/60 pt-3">
                  <div className="text-xs">
                    <span className="text-xs text-gray-400 font-extrabold uppercase tracking-wider block mb-0.5">Brand Kategori</span>
                    <span className="font-extrabold text-gray-800">{p.Category}</span>
                    <span className="text-gray-400 text-sm block font-medium">{p.Variant}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-xs text-pink-500 font-extrabold uppercase tracking-wider block mb-0.5">Jumlah Stok</span>
                    <span className={`inline-block px-2.5 py-0.5 rounded-full font-black ${
                ...
                <div className="flex justify-between items-center bg-pink-50/30 p-2.5 rounded-xl border border-pink-100/20 text-xs gap-3">
                  <div>
                    <span className="text-gray-400 text-xs block uppercase font-mono tracking-wider">Harga Jual</span>
                    <span className="font-extrabold text-gray-900">Rp {p.Retail_Price.toLocaleString()}</span>
                  </div>
                  {currentUser?.Role === 'Owner Alina' && (
                    <div className="text-right">
                      <span className="text-gray-400 text-xs block uppercase font-mono tracking-wider">Harga Modal</span>
                ...
                {urgent && (
                  <div className="bg-red-50 border border-red-100 text-red-600 text-sm font-bold p-2.5 rounded-xl flex items-center gap-1.5">
