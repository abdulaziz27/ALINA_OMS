...
export default function OrderDetailModal({
  ...
  else if (rep.Status === 'Ready To Ship') statClass = "bg-pink-50 text-pink-500 border border-pink-100 animate-pulse";
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[110] flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] w-full max-w-lg overflow-hidden text-left shadow-2xl border border-pink-100 p-6 space-y-4 max-h-[92vh] overflow-y-auto">
        <div className="flex justify-between items-start border-b border-pink-50 pb-3">
          <div>
            <span className="text-sm font-mono font-bold text-gray-400 block uppercase">Fulfillment Station Detail</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 bg-pink-50/50 rounded-2xl border border-pink-100/40 space-y-1">
            <p className="text-pink-400 font-bold uppercase text-xs tracking-wider">Identitas Pelanggan</p>
          ...
          <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 space-y-1">
            <p className="text-gray-400 font-bold uppercase text-xs tracking-wider">Informasi Transaksi</p>
            <p className="font-mono text-xs font-extrabold text-pink-600">{formatIDR(totalSum)}</p>
            <p className="text-sm text-gray-500 font-bold mt-1">🗓 {new Date(rep.Order_Date).toLocaleDateString()} {new Date(rep.Order_Date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
          </div>
        </div>

        {/* Order Status Display */}
        <div className="flex items-center justify-between p-3 bg-pink-50/40 rounded-2xl border border-pink-100/40 text-xs">
          <span className="text-gray-500 font-bold uppercase text-sm">Status Saat Ini:</span>
          <span className={`px-2.5 py-1 rounded text-sm uppercase font-black tracking-wider ${statClass}`}>
            {rep.Status}
          </span>
        </div>
        ...
        <div className="space-y-2 text-xs">
          <p className="font-bold text-gray-700 uppercase text-sm tracking-wider border-b border-pink-50 pb-1 flex justify-between">
            ...
            {relatedItems.map((item, idx) => (
              <div key={idx} className="pt-2 first:pt-0 flex justify-between items-start gap-4">
                <div>
                  ...
                  <span className="text-sm text-gray-505 font-bold font-mono">
        ...
        {(() => {
          ...
          return (
            <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 text-center flex flex-col items-center">
              <span className="text-xs font-mono font-bold text-gray-400 mb-2">IDENTIFIER DE FASHION CODE128 TAG</span>
              <svg width="220" height="48" viewBox={`0 0 ${width} 65`} className="mx-auto block">
                <path d={path} stroke="#111827" strokeWidth="2" />
              </svg>
              <span className="text-sm font-mono font-bold mt-1 tracking-widest text-pink-500">{activeDetailOrderNum}</span>
            </div>
          );
        })()}
        ...
        <div className="space-y-2 text-xs">
          <p className="font-bold text-gray-700 uppercase text-sm tracking-wider border-b border-pink-50 pb-1">
            Riwayat Aktivitas & Workflow Logs
          </p>
          <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
            {logs.length === 0 ? (
              <p className="text-gray-400 italic text-sm">Belum ada catatan aktivitas untuk pesanan ini.</p>
            ) : (
              <div className="border-l border-pink-100 pl-3.5 space-y-3 py-1 text-left">
                {logs.map((log, idx) => (
                  <div key={`${log.Log_ID || 'detail-log'}-${idx}`} className="relative">
                    <div className="absolute -left-[19.5px] top-1 w-2 h-2 rounded-full bg-pink-500 ring-4 ring-pink-50" />
                    <span className="text-xs font-mono font-bold bg-pink-50 text-pink-500 px-1.5 py-0.5 rounded inline-block mb-1">
                    ...
                    <span className="text-xs text-gray-400 font-medium">Petugas: {log.User_Name} ({log.User_Role})</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="flex flex-col gap-2 pt-2 border-t border-pink-50/60">
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-black py-2.5 px-4 rounded-xl cursor-pointer text-xs flex items-center justify-center gap-1.5 shadow-md uppercase tracking-wider transition"
            >
          ...
          {['New Order', 'Processing', 'Picking', 'Ready To Ship'].includes(rep.Status) && (
            <div className="bg-pink-50 rounded-xl border border-pink-100/40 p-2.5 flex items-center justify-between text-[11px]">
              ...
              <div>
                {rep.Status === 'New Order' && (
                  <button
                    ...
                    className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded-lg text-sm transition cursor-pointer"
                  >
                    Proses Pesanan →
                  </button>
                )}
                ...
                {rep.Status === 'Processing' && (
                  <button
                    ...
                    className="bg-purple-500 hover:bg-purple-600 text-white font-bold py-1 px-3 rounded-lg text-sm transition cursor-pointer"
                  >
                    Mulai Picking 🛒
                  </button>
                )}
                ...
                {rep.Status === 'Picking' && (
                  <button
                    ...
                    className="bg-pink-500 hover:bg-pink-600 text-white font-bold py-1 px-3 rounded-lg text-sm flex items-center gap-1 transition cursor-pointer"
                  >
