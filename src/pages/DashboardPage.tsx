    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-pink-100/60 p-4 rounded-[24px] shadow-sm flex flex-col justify-between text-left">
          <p className="text-sm font-mono uppercase tracking-wider text-gray-400 font-bold">Total Stock Pcs</p>
          ...
          <span className="text-xs bg-emerald-50 text-emerald-600 font-bold px-2 py-0.5 rounded-full mt-2 w-fit">Sehat</span>
        </div>

        <div className="bg-white border border-pink-100/60 p-4 rounded-[24px] shadow-sm flex flex-col justify-between text-left">
          <p className="text-sm font-mono uppercase tracking-wider text-gray-400 font-bold">Sku Terdaftar</p>
          ...
          <span className="text-xs bg-pink-100 text-pink-500 font-bold px-2 py-0.5 rounded-full mt-2 w-fit">Alina Mode</span>
        </div>

        <div className="bg-white border border-pink-100/60 p-4 rounded-[24px] shadow-sm flex flex-col justify-between text-left">
          <p className="text-sm font-mono uppercase tracking-wider text-gray-400 font-bold">Orders Today</p>
          ...
          <span className="text-xs bg-pink-50 text-pink-500 font-bold px-2 py-0.5 rounded-full mt-2 w-fit">Live</span>
        </div>

        <div className="bg-white border border-pink-100/60 p-4 rounded-[24px] shadow-sm flex flex-col justify-between text-left">
          <p className="text-sm font-mono uppercase tracking-wider text-gray-400 font-bold">Fulfillment Status</p>
          <div className="flex gap-1.5 text-sm font-bold mt-1 text-gray-800">
          ...
          <span className="text-xs text-gray-400 mt-2 block font-mono">This month total: {ordersThisMonthCount}</span>
        </div>
      </div>
      ...
      {currentUser.Role === 'Owner Alina' ? (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <span className="bg-pink-100 text-pink-500 text-xs font-black px-3 py-1 rounded-full uppercase">Secure Owner Financial Panel</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-pink-100 p-5 rounded-[32px] text-left space-y-3">
          <h4 className="font-bold text-xs uppercase tracking-wider text-gray-700 flex items-center gap-1.5">
            <AlertTriangle className="w-4 h-4 text-pink-500" /> ⚠️ ALINA STOCK ALERTS (Critical Levels)
          </h4>
            ...
            {products.filter(p => p.Current_Stock <= p.Minimum_Stock).map((p, pIdx) => (
              <div key={p.SKU || `warn-prod-${pIdx}`} className="py-2.5 flex justify-between items-center text-xs">
                <div>
                  ...
                  <p className="text-sm font-mono text-gray-400">SKU: {p.SKU} | Kategori: {p.Category}</p>
                </div>
                <div className="text-right">
                  <span className="bg-red-50 text-red-600 font-extrabold px-2.5 py-0.5 rounded-full block text-sm">
                    {p.Current_Stock} Pcs Left
                  </span>
                  <span className="text-xs text-gray-400">Threshold: {p.Minimum_Stock} Pcs</span>
                </div>
              </div>
            ))}
        ...
        <div className="bg-pink-50 border border-pink-100 p-5 rounded-[32px] text-left flex flex-col justify-between">
          ...
          <div className="space-y-3 text-xs leading-relaxed text-gray-600">
            ...
            <ul className="list-disc pl-5 space-y-1 text-gray-500 font-medium">
              <li>Fungsi <strong className="text-pink-500">QR Code</strong> otomatis dibuat per produk. Unduh dan tempel d tag baju.</li>
              <li>Gunakan modul <strong className="text-pink-500">Picking & Packing</strong> saat memproses pesanan untuk memverifikasi akurasi isi barang menggunakan laser terminal HP.</li>
              <li>Simpan <strong className="text-pink-500">Stock Opname bulanan</strong> pada tanggal akhir untuk menjaga konsistensi selisih stok fisik & sistem.</li>
              <li>Masukkan Spreadsheet ID Anda pada tab <strong className="text-pink-500">Sheets Connector</strong> untuk menyinkronkan data.</li>
            </ul>
          </div>
        </div>
